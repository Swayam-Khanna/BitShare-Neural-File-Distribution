...
import { Check, Zap, Rocket, Shield, ArrowRight, Sparkles, Globe } from 'lucide-react';
...
const PlanCard = ({ tier, price, storage, features, icon, highlighted = false, onSelect, onBypass, loading }: any) => (
  <div 
    className={`glass p-10 rounded-[3rem] border ${highlighted ? 'border-primary-500 bg-primary-500/5 shadow-2xl shadow-primary-500/20' : 'border-white/5'} relative overflow-hidden group flex flex-col h-full transition-all duration-500 hover:-translate-y-2 lg:hover:scale-[1.02] gpu-accel`}
  >
    {highlighted && (
    ...
    <button 
        ...
        disabled={loading}
        className={`w-full py-3 sm:py-4 rounded-2xl font-black transition-all flex items-center justify-center gap-3 active:scale-95 group/btn text-sm ${
            highlighted 
...
const Pricing = () => {
  ...
  return (
    <div className="relative min-h-screen text-white overflow-x-hidden selection:bg-primary-500/30 bg-slate-950">
      <Scene3D />
      <Navbar />
      ...
      <main className="relative z-10 pt-32 sm:pt-40 pb-20 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-12 lg:mb-20">
          ...
          <motion.h1 
            ...
            className="text-4xl sm:text-6xl font-black italic tracking-tighter uppercase mb-6"
          >
          ...
          <motion.p 
            ...
            className="text-gray-500 font-bold text-base sm:text-lg max-w-2xl mx-auto italic"
          >
